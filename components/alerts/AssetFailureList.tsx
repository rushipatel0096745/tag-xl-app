...
import { GetAssetFailure } from "@/services/alerts";
import { FailAssetItem } from "@/types/Alert";
...
const AssetFailureList = () => {
    const [list, setList] = useState<FailAssetItem[]>([]);
    ...
    async function fetchAlerts(pageNumber = 1, isLoadMore = false) {
        try {
            ...
            const res = await GetAssetFailure([], search, pageNumber, 20, 0);
    ...
    const renderItem = ({ item }: { item: FailAssetItem }) => {
...
export default AssetFailureList;
