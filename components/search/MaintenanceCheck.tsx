...
const MaintenanceCheck = () => {
    ...
    async function fetchAsset() {
        try {
            const result = await GetAsset(parseInt(asset_id as string));
            // const result = await GetAsset(26);
            ...
            let maintenanceQuestions = asset?.maintenance_template?.questions || [];
            if (asset?.asset_maintenance_questions) {
                maintenanceQuestions.push(...asset.asset_maintenance_questions);
            }
            setQuestions(maintenanceQuestions);

            if (maintenanceQuestions.length > 0) {
                setActiveQuestionId(maintenanceQuestions[0].id);
            }
        } catch (err) {
